...
- **HTTP Proxy**: `proxy.Manager` runs per-instance listeners, each with its own `http.Server` bypassing `LoggingMiddleware` (which buffers whole bodies). Two modes per instance: `reverse` (fixed target, `httputil.ReverseProxy` with WS upgrade passthrough) and `forward` (client-named destinations via absolute URI/CONNECT, authenticated with the config super admin via Proxy-Authorization). Instances come from `proxy.instances` in config or the `/api/proxy/instances` API (SQLite-persisted, restored on restart). Target access is gated by a deny-by-default whitelist with SSRF guards (private ranges need `allow_private`, cloud metadata endpoints always denied, post-dial IP re-check); forward mode requires a non-empty whitelist. Poll-mode `proxy_session` tasks reuse the same whitelist-checked transport through `proxy.TunnelCore`. See `docs/proxy.md`.
