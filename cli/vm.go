...
func (c *CLI) handleVMFileUpload(sourcePath, targetPath string) error {
	...
	// 上传文件
	if err := c.client.FileUpload(selectedVM.Name, sourcePath, targetPath, password); err != nil {
...
func (c *CLI) handleVMFileDownload(sourcePath, targetPath string) error {
	...
	// 下载文件
	if err := c.client.FileDownload(selectedVM.Name, sourcePath, targetPath, password); err != nil {
...
func (c *CLI) handleVMFileList(path string, maxDepth int) error {
	...
	// 获取文件列表
	files, err := c.client.FileList(selectedVM.Name, path, maxDepth, password)
	if err != nil {
