...
// SSHConnect SSH连接到VM
func (c *Client) SSHConnect(host string, port int, username, password, keyPath string) error {
	...
	resp, err := c.doRequest("POST", "/api/vms/ssh/connect", sshConfig)
	if err != nil {
		return err
	}
...
func (c *Client) SSHExecute(host string, port int, username, password, keyPath, command string) (string, error) {
	reqBody := map[string]interface{}{
		"ssh_config": vm.SSHConfig{
			Host:     host,
			Port:     port,
			Username: username,
			Password: password,
...
// FileUpload 上传文件到VM
func (c *Client) FileUpload(host string, port int, username, password, keyPath, localPath, remotePath string) error {
	reqBody := map[string]interface{}{
		"ssh_config": vm.SSHConfig{
			Host:     host,
			Port:     port,
	...
	resp, err := c.doRequest("POST", "/api/vms/file/upload", reqBody)
	if err != nil {
		return err
	}
...
func (c *Client) FileDownload(host string, port int, username, password, keyPath, remotePath, localPath string) error {
	reqBody := map[string]interface{}{
		"ssh_config": vm.SSHConfig{
			Host:     host,
			Port:     port,
...
// FileList 列出VM上的文件
func (c *Client) FileList(host string, port int, username, password, keyPath, remotePath string) ([]vm.FileInfo, error) {
