...
// ManageProcess implements ProcessManager interface to manage a process
...
// StartProcess implements ProcessManager interface to start a process
	...
	if runtime.GOOS == "windows" {
		...
		if err := sysProcess.Signal(syscall.SIGTERM); err != nil {
			...
			for {
				...
				default:
					// 检查进程是否存在
					syscall.Kill(process.PID, 0) // 发送0信号检查进程是否存在
